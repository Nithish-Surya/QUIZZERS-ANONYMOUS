...
//  root.renderer.setClearColor(0x000000);
  ...
  // Fill light from the side (soft blue tint)
  var fillLight = new THREE.DirectionalLight(0x8888ff, 0.4);
  fillLight.position.set(-1, 0.5, -1);
  root.scene.add(fillLight);

  // Ambient so no face goes pitch black
  var ambientLight = new THREE.AmbientLight(0x444444);
  root.scene.add(ambientLight);

  // Only plays once: from Ball (1.0) -> Text (0.0)
...
function createTextAnimation() {
  var geometry = generateTextGeometry('QUIZZERS ANONYMOUS', {
    size: 60,           //40
    height: 14,
    font: 'droid sans',     // kept original font — no extra file needed
    weight: 'bold',
    style: 'normal',
    curveSegments: 64,      // was 24 — smoother curves on all letters
    bevelSize: 1,           // was 2 — tighter, crisper bevel
    bevelThickness: 1,      // was 2
    bevelEnabled: true,
    anchor: {
      x: 0.5,
      y: 0.5,
  ...
  THREE.BAS.Utils.tessellateRepeat(geometry, 1.0, 4); // was 2 — finer triangulation
...
function TextAnimation(textGeometry) {
  ...
  var material = new THREE.BAS.PhongAnimationMaterial({
    ...
    diffuse:   0xffffff,   // was 0x444444 — full white base
    specular:  0xffffff,   // was 0xcccccc — bright highlights
    shininess: 60          // was 4 — sharp, solid-looking letters
  });
